...
public class ScheduledThreadPoolExecutor extends ThreadPoolExecutor implements ScheduledExecutorService {
    ...
    private class ScheduledFutureTask<V> extends FutureTask<V> implements RunnableScheduledFuture<V> {
        ...
        /**
         * period == 0 当前任务是一次性的
         * period > 0  固定延迟时间可重复执行任务（fixed-delay）
         * period < 0  固定频率可重复执行任务    (fixed-rate)
         */
    ...
    public ScheduledFuture<?> scheduleAtFixedRate(Runnable command, long initialDelay, long period, TimeUnit unit) {
        ...
        // 任务转换：将 command 任务转换为  ScheduledFutureTask
    ...
    static class DelayedWorkQueue extends AbstractQueue<Runnable> implements BlockingQueue<Runnable> {
        ...
        // 使用 lock 来保证多线程并发安全问题
        private final ReentrantLock lock = new ReentrantLock();
        // 队列中村粗元素的大小
        private int size = 0;
        // 队头任务所在 leader 线程
        private Thread leader = null;
        // 当队头的任务延时时间到来 || 或者新线程要变成 leader ,用来唤醒等待线程
        ...
        public boolean offer(Runnable x) {
            ...
            try {
                ...
                if (i >= queue.length)
                    grow();  // 数组扩容
                size = i + 1;
                if (i == 0) { // 如果是第一个元素 不用排序 直接赋值就行
                    queue[0] = e;
                    setIndex(e, 0);
                } else {
                    siftUp(i, e); // 排序
                }
                if (queue[0] == e) { // 唤醒正在等待获取任务的想吃
                    leader = null;
                    available.signal();
                }
            } finally {
